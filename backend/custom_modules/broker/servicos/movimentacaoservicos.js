...
router.route('/filtrarImportacaoBySisco/:dataDe/:dataAte/:cliente/:servico/:cotacao').get(function(req, res) {
    ...
    if(servico == "*" || servico == "RVS"){
        ...
        query += " SELECT nrrvs AS numcod, REPLACE(venda.codigo, '/', '-' ) AS idvenda,venda.codigo AS codigo, ";
    ...
    query += " venda.informacoescomplementares, vendaoperacao.suareferencia, venda.idvenda, nrrvs ";
    ...
    if(servico == "*" || servico == "RAS"){
        query += " SELECT nrras AS numcod, REPLACE(aquisicao.codigo, '/', '-' ) AS idvenda,aquisicao.codigo AS codigo, ";
    ...
    query += " aquisicao.informacoescomplementares, aquisicaooperacao.suareferencia,aquisicao.idaquisicao , nrras";
    ...
    if(servico == "*" || servico == "RF"){
        query += " SELECT  nrrf AS numcod, REPLACE(faturamento.codigo, '/', '-' ) AS idvenda,faturamento.codigo AS codigo, ";
    ...
    query += " faturamento.observacoes,  venda.idvenda, nrrf ";
    ...
    if(servico == "*" || servico == "RP"){
        query += " SELECT  nrrp AS numcod, REPLACE(pagamento.codigo, '/', '-' ) AS idvenda,pagamento.codigo AS codigo, ";
    ...
    query += " pex.nome , pessoa.cpfcnpj, pagamento.observacoes, aquisicao.idaquisicao, nrrp  ";
                                ...
                                if(recordset.recordsets.length > 0){
                                    ...
                                    if (element.nr_diavencimento != null && element.nr_diavencimento != 0){
                                        ...
                                        if (element.nr_diavencimento > hoje.getDate() && element.sn_messeguinte == false)
                                            ...
                                            dataInicial = element.sn_messeguinte != null && element.sn_messeguinte ? dataInicial.setMonth(dataInicial.getMonth() + 2) : dataInicial;
                                    }
                                    else{
...
router.route('/gerarContasReceber').post(function(req, res) {     
    ...
    try{
        ...
        sql.connect(config, function (err) {
            ...
            query += " entidade.sn_issretido AS 'issretido', ";
            query += " entidade.vl_issretido  AS 'aliqissretido',  ";
            query += " entidade.sn_pccretido AS 'sn_pccretido', ";

            query += " ((SELECT SUM(vl_valor)  ";
            query += " FROM contas_receber  ";
            query += " WHERE id_entidade=entidade.id  ";
            query += " AND dt_emissao >=  CONVERT(datetime,CONVERT(varchar,MONTH(GETDATE())) + '/' + '01/' +  CONVERT(varchar,YEAR(GETDATE())))   ";
            query += " AND dt_emissao <= EOMONTH(GETDATE()))) AS 'creditoatual', ";

            query += " (SELECT configuracao_nfe_servico.vl_limite_retencao FROM configuracao_nfe_servico) AS 'limiteretencao', ";
            query += " (SELECT configuracao_nfe_servico.vl_aliq_pis_cofins_csll FROM configuracao_nfe_servico) AS 'aliqpcc', ";
                                        ...
                                        for (let h = 0; h < recordset.recordsets[0].length; h++) {
                                            ...
                                            if(movimentacao.sn_pccretido && (parseFloat(movimentacao.valortotal) * pcc / 100) >= 10 ){
                                                if(credatual + parseFloat(movimentacao.valortotal) >= limite){
                                                    if(credatual < limite){
                                                        tot = tot - ((credatual + parseFloat(movimentacao.valortotal)) * ir / 100)
                                                    }else{
                                                        tot = tot - (parseFloat(movimentacao.valortotal) * ir / 100);   
                                                    }
                                                }

                                                tot = tot - (parseFloat(movimentacao.valortotal) * pcc / 100);                                                 
                                            }
                                            ...
                                            for(i = 0; i < respostaParcelas.parcelas.length; i++){
                                                ...
                                                total += tot; //parseFloat(movimentacao.valortotal);
                                                titulo.parcelas.push(parcela);
                                            }                        
    ...
    try{
        for (let j = 0; j < Aparametros.length; j++) {
            ...
            if(resposta.status == 1){
                ...
                queryItens += "insert into contas_receber_parcelas (id,id_empresa,id_contas_receber,id_Banco,id_forma_pagamento,id_plano_contas_financeiro,nr_parcela,nm_documento,sn_fluxocaixa,dt_data_vencimento,vl_valor, vl_valortotal, id_configuracao_cnab)";
                queryItens += " values ";
                for(parcela = 0; parcela < parametros.parcelas.length; parcela++){
                    if(parcela > 0)
                        queryItens += ",";
                    ...
                    queryItens += "'" + parametros.parcelas[parcela].vencimento + "',";
                    queryItens += parametros.parcelas[parcela].valor.toString().trim() + ",";
                    queryItens += parametros.parcelas[parcela].valortotal.toString().trim() + ",";
...
router.route('/gerarNFSe').post(function(req, res) {
    ...
    try {
        ...
        if(parametros.competencia){
            var arraycompetencia = parametros.competencia.split("/");
            if(arraycompetencia.length == 3){
                parametros.competencia = arraycompetencia[2] + "-" + arraycompetencia[1] + "-" + arraycompetencia[0];
            }
        }
        ...
        select += " WHERE id_entidade=entidade.id ";
        select += " AND dt_emissao >=  CONVERT(datetime,'01/' + CONVERT(varchar,MONTH(GETDATE())) + '/' +  ";
        select += " CONVERT(varchar,YEAR(GETDATE())))  ";
        select += " AND dt_emissao <= EOMONTH(GETDATE()))) >= configuracao_nfe_servico.vl_limite_retencao,  ";
        ...
        select += " AND dt_emissao >=  CONVERT(datetime,'01/' + CONVERT(varchar,MONTH(GETDATE())) + '/' +  ";
        select += " CONVERT(varchar,YEAR(GETDATE())))  ";
        select += " AND dt_emissao <= EOMONTH(GETDATE())) - ";
        select += " SUM(movimentacao_servicos.vl_valor) ) < configuracao_nfe_servico.vl_limite_retencao, ";
        ...
        select += " AND dt_emissao >=  CONVERT(datetime,'01/' + CONVERT(varchar,MONTH(GETDATE())) + '/' +  ";
        select += " CONVERT(varchar,YEAR(GETDATE())))  ";
        select += " AND dt_emissao <= EOMONTH(GETDATE())) * configuracao_nfe_servico.vl_aliq_irrf / 100), ";
        ...
        select += " LEFT JOIN configuracao_nfe_servico ON configuracao_nfe_servico.id_empresa='9F39BDCF-6B98-45DE-A819-24B7F3EE2560'  ";
                    ...
                    transacao.begin(err =>{
                        for (let h = 0; h < recordset.recordsets[0].length; h++) {
                            ...
                            */
                            ...
                            var ValorIRRF = movimentacao.ValorIRRF
                            ValorIRRF = (!ValorIRRF) ? "0.00" : ValorIRRF;

                            var ValorCSLL = movimentacao.ValorCSLL
                            ValorCSLL = (!ValorCSLL) ? "0.00" : ValorCSLL;
                            ...
                            var AliquotaIRRF = movimentacao.AliquotaIRRF
                            AliquotaIRRF = (!AliquotaIRRF) ? "0.00" : AliquotaIRRF;
                            
                            var AliquotaCSLL = movimentacao.AliquotaCSLL
                            AliquotaCSLL = (!AliquotaCSLL) ? "0.00" : AliquotaCSLL;
                            ...
                            //if(status == "Pendente" || !status){
                                ...
                                query += " IDProdutos_VendaProdutos, SerieRpsSubstituido, status, dataemissao, DescricaoNota, ValorIr, AliquotaIR, AliquotaCSLL, ValorCsll, ValorContasReceber , DataVencimento, Competencia) ";
...
router.route('/getInfoNFSe').post(function(req, res) {
    ...
    select += " AliquotaPIS, AliquotaCOFINS, IDProdutos_VendaProdutos, SerieRpsSubstituido , DescricaoNota, ValorIr, AliquotaIR, AliquotaCSLL, ValorCsll, ValorContasReceber, DataVencimento, Competencia ";
    select += " FROM nfse ";
    select += " WHERE ";
            ...
            if(retorno){
                    ...
                    for (let i = 0; i < parametros.codigo.length; i++) {
                        ...
                        if(status == "Pendente" || !status){
                            ...
                            insert += " nm_numero_nfes,nm_numero_boleto,dt_faturamento,nm_numero_operacao,nm_status,id_contas_receber, nm_nomepessoaextrangeira, id_referencia, nm_numcod) ";
                            ...
                            insert += " NULL, NULL, NULL, '" + parametros.codigo[i] + "', NULL, NULL, '" + parametros.nomepessoaextrangeira[i] + "', '" + parametros.idreferencia[i] + "', '" + parametros.numcod[i] + "'";
                            insert += "); ";
                        }
